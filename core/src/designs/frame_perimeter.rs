...
use super::{
    Design, DesignConfig, DesignParams, DesignType, EdgeDistribution, FramePerimeterParams,
    QuadData, Rect, RenderContext, Vertex,
};
    ...
    fn generate_perimeter(
        ...
        for (i, &value) in spectrum.iter().enumerate() {
            ...
            let (edge, t) = if pos < ctx.width {
                ...
                (
                    Edge::Bottom,
                    1.0 - (pos - ctx.width - ctx.height) / ctx.width,
                )
            } else {
                (
                    Edge::Left,
                    1.0 - (pos - 2.0 * ctx.width - ctx.height) / ctx.height,
                )
            };

            let bar_length = max_length * value * ctx.beat_scale;
            let (x1, y1, x2, y2) =
                edge.bar_rect(t, ctx, params.inset, bar_width, bar_length, params.inward);
            ctx.push_quad(
                vertices,
                QuadData {
                    bounds: Rect::new(x1, y1, x2, y2),
                    value,
                    index: i as f32,
                },
            );
