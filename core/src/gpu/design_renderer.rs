...
impl DesignRenderer {
    ...
    pub async fn new(config: DesignRenderConfig) -> Result<Self, super::context::GpuError> {
        ...
        let pipeline_layout = create_pipeline_layout(&ctx.device, "design_pipeline_layout", &[&bind_group_layout]);
        ...
        let output_target = RenderTarget::for_output(&ctx.device, "design_output", config.width, config.height, format);

        // Glow resources (only when enabled)
        let glow = if config.glow {
            Some(GlowResources {
                scene_target: RenderTarget::for_scene(&ctx.device, "design_scene", config.width, config.height, format),
                postprocess: PostProcessPipeline::new(&ctx.device, PostProcessConfig {
                    width: config.width,
        ...
        Ok(Self { ctx, pipeline, bind_group, uniform_buffer, vertex_buffer, output_target, glow, config, design, max_vertices })
    }
    ...
    pub fn render_frame(&self, spectrum: &[f32], beat_intensity: f32) -> Vec<u8> {
        let vertices = self.design.generate_vertices(spectrum, &DesignConfig {
        ...
        // Update GPU state
        self.ctx.queue.write_buffer(&self.uniform_buffer, 0, bytemuck::bytes_of(&DesignUniforms {
            color: [self.config.color[0], self.config.color[1], self.config.color[2], 1.0],
            beat_intensity,
        ...
        if !vertices.is_empty() {
            self.ctx.queue.write_buffer(&self.vertex_buffer, 0, bytemuck::cast_slice(&vertices[..vertex_count]));
        }

        let mut encoder = self.ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("render") });
        ...
        if needs_postprocess {
            if let Some(glow) = &self.glow {
                glow.postprocess.apply(&self.ctx.device, &self.ctx.queue, &mut encoder, glow.scene_target.view(), self.output_target.view(), beat_intensity);
            }
        }
    ...
    fn run_render_pass(&self, encoder: &mut wgpu::CommandEncoder, target: &TextureView, vertex_count: usize) {
...
mod tests {
    ...
    where F: FnOnce(&DesignRenderer, &DesignRenderConfig)
    {
    ...
    async fn test_all_design_types_render_correct_size() {
        for (design_type, design_params) in [
            ...
            with_renderer(DesignRenderConfig { width: 256, height: 256, bar_count: 32, design_type, design_params, ..Default::default() }, |renderer, config| {
                let pixels = renderer.render_frame(&(0..32).map(|i| i as f32 / 32.0).collect::<Vec<_>>(), 0.5);
    ...
    async fn test_background_color_rendering() {
        with_renderer(DesignRenderConfig { width: 64, height: 64, bar_count: 4, background: [0.0, 0.0, 0.0], glow: false, ..Default::default() }, |r, _| {
            let p = r.render_frame(&vec![0.0; 4], 0.0);
            assert!(p[0] < 10 && p[1] < 10 && p[2] < 10);
        }).await;

        with_renderer(DesignRenderConfig { width: 64, height: 64, bar_count: 4, background: [1.0, 1.0, 1.0], glow: false, ..Default::default() }, |r, _| {
            let p = r.render_frame(&vec![0.0; 4], 0.0);
    ...
    async fn test_beat_intensity_changes_output() {
        with_renderer(DesignRenderConfig { width: 128, height: 128, bar_count: 8, glow: false, ..Default::default() }, |r, _| {
            assert_ne!(r.render_frame(&vec![0.5; 8], 0.0), r.render_frame(&vec![0.5; 8], 1.0));
        }).await;
    }
    ...
    async fn test_glow_changes_output() {
        match (
            DesignRenderer::new(DesignRenderConfig { width: 128, height: 128, bar_count: 8, glow: true, ..Default::default() }).await,
            DesignRenderer::new(DesignRenderConfig { width: 128, height: 128, bar_count: 8, glow: false, ..Default::default() }).await,
        ) {
            (Ok(r1), Ok(r2)) => assert_ne!(r1.render_frame(&vec![0.8; 8], 0.0), r2.render_frame(&vec![0.8; 8], 0.0)),
    ...
    async fn test_multiple_frames_consistent() {
        with_renderer(DesignRenderConfig { width: 128, height: 128, bar_count: 16, ..Default::default() }, |r, c| {
            for i in 0..5 {
                assert_eq!(r.render_frame(&(0..16).map(|j| ((i + j) as f32 / 20.0).min(1.0)).collect::<Vec<_>>(), i as f32 / 5.0).len(), (c.width * c.height * 4) as usize);
            }
        }).await;
    }
    ...
    async fn test_empty_spectrum_renders() {
        with_renderer(DesignRenderConfig { width: 64, height: 64, bar_count: 8, ..Default::default() }, |r, c| {
            assert_eq!(r.render_frame(&[], 0.0).len(), (c.width * c.height * 4) as usize);
        }).await;
    }
}
