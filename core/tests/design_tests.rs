...
fn test_bars_mirror_changes_scaling() {
    ...
    let v_normal = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::Bars(BarsParams {
            mirror: false,
            ..Default::default()
        }),
    );
    let v_mirror = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::Bars(BarsParams {
            mirror: true,
...
fn test_circular_radial_full_circle() {
    ...
    assert!(
        ...
        "Full circle should cover all quadrants"
    );
}
...
fn test_circular_radial_partial_arc() {
    ...
    let v_full = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::CircularRadial(CircularRadialParams::default()),
    );
    let v_half = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::CircularRadial(CircularRadialParams {
            arc_span: PI,
            ..Default::default()
        }),
    );
...
fn test_circular_radial_varying_radii() {
    ...
    let v_large = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::CircularRadial(CircularRadialParams {
            inner_radius: 0.1,
    ...
    let v_small = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::CircularRadial(CircularRadialParams {
            inner_radius: 0.4,
...
fn test_circular_ring_inward_vs_outward() {
    ...
    let v_outward = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::CircularRing(CircularRingParams {
            radius: 0.35,
            bar_length: 0.15,
            inward: false,
            rotation: 0.0,
    ...
    let v_inward = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::CircularRing(CircularRingParams {
            radius: 0.35,
...
fn test_circular_ring_rotation() {
    ...
    let v_no_rot = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::CircularRing(CircularRingParams {
            rotation: 0.0,
    ...
    let v_quarter = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::CircularRing(CircularRingParams {
            rotation: PI / 2.0,
...
fn test_frame_perimeter_top_bottom_distribution() {
    ...
    assert!(
        has_top && has_bottom,
        "TopBottom should have bars at top and bottom"
    );
}
...
fn test_frame_perimeter_inward_vs_outward() {
    ...
    let v_inward = design.generate_vertices(
        &spectrum,
        &config,
        &DesignParams::FramePerimeter(FramePerimeterParams {
    ...
    let avg_y_inward: f32 =
        v_inward.iter().map(|v| v.position[1]).sum::<f32>() / v_inward.len() as f32;
    let avg_y_outward: f32 =
        v_outward.iter().map(|v| v.position[1]).sum::<f32>() / v_outward.len() as f32;

    assert!(
        avg_y_inward < avg_y_outward,
        "Inward bars should have lower average Y"
    );
}
    ...
    for design_type in DesignType::all() {
        ...
        for (i, v) in vertices.iter().enumerate() {
            assert!(
                ...
                "Design {:?} vertex {} invalid local_pos[0]",
                design_type,
                i
            );
            assert!(
                v.local_pos[1].abs() <= 2.0,
                "Design {:?} vertex {} invalid local_pos[1]",
                design_type,
                i
            );
        }
...
fn test_all_designs_handle_max_spectrum() {
    ...
    for design_type in DesignType::all() {
        ...
        for v in &vertices {
            assert_eq!(
                v.bar_height, 1.0,
                "Max spectrum should produce max bar heights"
            );
        }
    }
}
