...
use phobz_visualizer::designs::{DesignConfig, DesignType, create_design, default_params};
...
pub fn verify_design_generates_vertices(design_type: DesignType, spectrum_size: usize) {
    ...
    assert!(!vertices.is_empty(), "Design {:?} generated no vertices", design_type);
}

/// Check if vertices cover all quadrants (for circular designs).
pub fn vertices_cover_all_quadrants(vertices: &[phobz_visualizer::Vertex]) -> (bool, bool, bool, bool) {
    ...
    for v in vertices {
        if v.position[0] > 0.1 { has_pos_x = true; }
        if v.position[0] < -0.1 { has_neg_x = true; }
        if v.position[1] > 0.1 { has_pos_y = true; }
        if v.position[1] < -0.1 { has_neg_y = true; }
    }
...
pub fn x_position_range(vertices: &[phobz_visualizer::Vertex]) -> (f32, f32) {
    ...
    (
        ...
        x_positions.iter().cloned().fold(f32::NEG_INFINITY, f32::max),
    )
}
...
pub fn y_position_range(vertices: &[phobz_visualizer::Vertex]) -> (f32, f32) {
    ...
    (
        ...
        y_positions.iter().cloned().fold(f32::NEG_INFINITY, f32::max),
    )
}
